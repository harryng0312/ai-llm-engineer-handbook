...
> Chương này ghép LLM ([06-Transformer](../06-Transformer/06-Transformer.md)) và Vector Database ([08-Embedding](../08-Embedding/08-Embedding.md)) lại thành một pipeline hoàn chỉnh: cho LLM "tra cứu" tài liệu riêng trước khi trả lời, thay vì chỉ dựa vào kiến thức đã train.

> 📓 Toàn bộ code ví dụ trong chương này cũng có ở dạng notebook chạy được: [`09-RAG.ipynb`](./09-RAG.ipynb).

## Mục lục

1. [RAG là gì và tại sao cần](#chương-91--rag-là-gì-và-tại-sao-cần)
2. [Chunking](#chương-92--chunking)
3. [Embedding & Indexing](#chương-93--embedding--indexing)
4. [Retriever: dense, sparse, hybrid](#chương-94--retriever-dense-sparse-hybrid)
5. [Re-ranking](#chương-95--re-ranking)
6. [Ráp thành pipeline hoàn chỉnh](#chương-96--ráp-thành-pipeline-hoàn-chỉnh)
7. [Evaluation](#chương-97--evaluation)
8. [Bài tập](#chương-98--bài-tập)
9. [Tài liệu tham khảo](#chương-99--tài-liệu-tham-khảo)

---

## Chương 9.1 — RAG là gì và tại sao cần
...
Trong thực tế, đa số hệ thống production bắt đầu với fixed-size + overlap (rẻ, dễ đoán, đủ tốt) và chỉ chuyển sang semantic chunking khi đo được retrieval quality chưa đạt (xem [mục 7](#chương-97--evaluation)).

## Chương 9.3 — Embedding & Indexing

Sau khi có chunks, dùng model embedding ([chương 6](../08-Embedding/08-Embedding.md)) để encode và lưu vào vector DB. Payload nên giữ lại **text gốc** và metadata (nguồn, số trang...) để khi retrieve xong còn dùng được:
...
## Chương 9.4 — Retriever: dense, sparse, hybrid
...
## Chương 9.6 — Ráp thành pipeline hoàn chỉnh

Ghép chunking → indexing → hybrid retrieval → re-ranking → generation (qua Ollama/Qwen như ở [chương 4](../06-Transformer/06-Transformer.md)):
...
2. Cài đặt hybrid retrieval bằng Reciprocal Rank Fusion ở [mục 4](#chương-94--retriever-dense-sparse-hybrid), so sánh recall@5 với dense-only trên một bộ câu hỏi có chứa thuật ngữ/mã số chính xác.
3. Thêm bước re-ranking vào pipeline ở [mục 6](#chương-96--ráp-thành-pipeline-hoàn-chỉnh), đo thời gian tăng thêm và so sánh chất lượng câu trả lời trước/sau khi có re-ranking.
...
## Chương 9.9 — Tài liệu tham khảo

> 📖 Xem chú giải chi tiết "vì sao mỗi paper quan trọng" ở [17-Papers §4](../17-Papers/17-Papers.md#chương-174--rag) (nền tảng) và [§9](../17-Papers/17-Papers.md#chương-179--rag--agent-nâng-cao) (Self-RAG, CRAG — RAG nâng cao).
