...
Chương [11-Agent.md](../11-Agent/11-Agent.md) đã xây agent từ nền tảng thấp nhất: vòng lặp ReAct viết tay và tool calling thô qua JSON schema. **LangChain** là framework abstraction giúp không phải viết lại những phần lặp đi lặp lại đó — chuẩn hóa cách gọi nhiều provider LLM, cách định nghĩa tool, cách quản lý prompt — xây trên đúng nền tảng agent cơ bản đã học ở chương trước. Chương sau, [10-LangGraph.md](../10-LangGraph/10-LangGraph.md), sẽ đi sâu vào phần orchestration phức tạp hơn của cùng hệ sinh thái (đồ thị trạng thái, checkpoint, multi-agent).

> 📓 Toàn bộ code ví dụ trong chương này cũng có ở dạng notebook chạy được: [`09-LangChain.ipynb`](./09-LangChain.ipynb).

## Mục lục

1. [LangChain là gì](#chương-91--langchain-là-gì)
2. [Định nghĩa Tool bằng @tool](#chương-92--định-nghĩa-tool-bằng-tool)
3. [Structured Output: Pydantic và JSON Schema](#chương-93--structured-output-pydantic-và-json-schema)
4. [Bài tập](#chương-94--bài-tập)
5. [Tài liệu tham khảo](#chương-95--tài-liệu-tham-khảo)

---

## Chương 9.1 — LangChain là gì
...
## Chương 9.2 — Định nghĩa Tool bằng @tool

Định nghĩa tool bằng decorator `@tool` (gọn hơn viết JSON schema tay như ở [11-Agent.md](../11-Agent/11-Agent.md) — LangChain tự sinh schema từ type hint và docstring):
...
`create_react_agent` (từ `langgraph.prebuilt`) đã đóng gói sẵn vòng lặp ReAct hoàn chỉnh: model quyết định gọi tool nào, LangGraph tự chạy tool, đưa kết quả trở lại, lặp lại cho tới khi model trả lời cuối cùng — tương đương với đoạn code thủ công ở [11-Agent.md](../11-Agent/11-Agent.md) nhưng không phải tự viết vòng lặp.

## Chương 9.3 — Structured Output: Pydantic và JSON Schema
...
Structured output là nền tảng cho nhiều kỹ thuật ở các chương sau: planner ở [11-Agent.md](../11-Agent/11-Agent.md) cần output là 1 danh sách bước có cấu trúc, agent evaluation ở [11-Agent.md](../11-Agent/11-Agent.md) cần input/output có schema cố định để so sánh tự động.

## Chương 9.4 — Bài tập

1. Định nghĩa 1 schema Pydantic cho một task thật bạn hay làm (ví dụ trích xuất thông tin từ email), dùng `with_structured_output` ở [Chương 9.3](#chương-93--structured-output-pydantic-và-json-schema), thử với 5 input khác nhau và kiểm tra tỉ lệ parse thành công.
2. Viết 1 tool mới bằng `@tool` cho một use case khác — ví dụ `convert_currency(amount: float, from_currency: str, to_currency: str) -> float` (giả lập bảng tỷ giá cố định). Gắn tool này vào `create_react_agent` cùng với `get_weather`/`calculate` đã có ở [Chương 9.2](#chương-92--định-nghĩa-tool-bằng-tool), rồi thử hỏi 1 câu buộc agent phải tự chọn đúng tool trong 3 tool có sẵn.

## Chương 9.5 — Tài liệu tham khảo
