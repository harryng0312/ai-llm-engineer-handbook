# 17 — Papers: Danh sách đọc có chú giải
...
1. [Cách dùng chương này](#chương-171--cách-dùng-chương-này)
2. [Nền tảng Transformer & LLM](#chương-172--nền-tảng-transformer--llm)
3. [Embedding & Vector Search](#chương-173--embedding--vector-search)
4. [RAG](#chương-174--rag)
5. [Agent & Reasoning](#chương-175--agent--reasoning)
6. [Alignment & Fine-tuning nâng cao](#chương-176--alignment--fine-tuning-nâng-cao)
7. [Hiệu năng & Serving nâng cao](#chương-177--hiệu-năng--serving-nâng-cao)
8. [Lịch sử & Scaling Laws](#chương-178--lịch-sử--scaling-laws)
9. [RAG & Agent nâng cao](#chương-179--rag--agent-nâng-cao)
10. [Xem trước: Diffusion & Multimodal](#chương-1710--xem-trước-diffusion--multimodal)
11. [Cách đọc một paper AI/ML hiệu quả](#chương-1711--cách-đọc-một-paper-aiml-hiệu-quả)
12. [Bảng tổng hợp theo dòng thời gian](#chương-1712--bảng-tổng-hợp-theo-dòng-thời-gian)
13. [Tài liệu tham khảo](#chương-1713--tài-liệu-tham-khảo)

---

## Chương 17.1 — Cách dùng chương này
...
## Chương 17.2 — Nền tảng Transformer & LLM

*Song song với [06-Transformer.md](../06-Transformer/06-Transformer.md) (kiến trúc gốc, tokenization, decoder-only hiện đại).*

- **[Bắt buộc] Vaswani et al., *Attention Is All You Need* (2017)** — paper gốc giới thiệu kiến trúc Transformer, nền tảng của mọi LLM hiện đại. Đọc kỹ Hình 1 (kiến trúc encoder-decoder đầy đủ) cùng phần "Kiến trúc Transformer gốc" trong [06-Transformer.md](../06-Transformer/06-Transformer.md), rồi phần công thức scaled dot-product attention cùng phần "Kiến trúc Decoder-only hiện đại".
- **[Nên đọc] Sennrich et al., *Neural Machine Translation of Rare Words with Subword Units* (BPE, 2015)** — thuật toán Byte-Pair Encoding, nền tảng tokenizer của GPT/Llama/Qwen. Xem cài đặt tối giản ở phần "Tokenization" trong [06-Transformer.md](../06-Transformer/06-Transformer.md).
...
- **[Bắt buộc] Su et al., *RoFormer: Enhanced Transformer with Rotary Position Embedding* (2021)** — nguồn gốc của RoPE, kỹ thuật mã hóa vị trí dùng trong hầu hết LLM hiện đại (Llama, Qwen). Đọc cùng phần "RoPE" trong [06-Transformer.md](../06-Transformer/06-Transformer.md).
- **[Nên đọc] Ainslie et al., *GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints* (2023)** — giải thích cách "chuyển đổi" một model MHA có sẵn sang GQA mà không cần train lại từ đầu, chi tiết hơn phần tóm tắt ở phần "GQA/MQA" trong [06-Transformer.md](../06-Transformer/06-Transformer.md).
- **[Bắt buộc] Hu et al., *LoRA: Low-Rank Adaptation of Large Language Models* (2021)** — kỹ thuật fine-tune tiết kiệm tham số đứng sau hầu hết công cụ fine-tune LLM ngày nay. Xem thực hành ở [10-Fine-Tuning.md](../10-Fine-Tuning/10-Fine-Tuning.md).
- **[Nên đọc] Dettmers et al., *QLoRA: Efficient Finetuning of Quantized LLMs* (2023)** — kết hợp lượng tử hóa 4-bit với LoRA, lý do một model 7B fine-tune được trên GPU 8-12GB. Xem [10-Fine-Tuning.md](../10-Fine-Tuning/10-Fine-Tuning.md).
- **[Nên đọc] Kwon et al., *Efficient Memory Management for Large Language Model Serving with PagedAttention* (2023)** — paper của vLLM, giải thích PagedAttention chi tiết hơn sơ đồ tóm tắt ở phần "KV Cache" trong [06-Transformer.md](../06-Transformer/06-Transformer.md) và phần Serving trong [14-Deployment.md](../14-Deployment/14-Deployment.md).

## Chương 17.3 — Embedding & Vector Search

*Song song với [08-Embedding.md](../08-Embedding/08-Embedding.md).*

- **[Bắt buộc] Oord et al., *Representation Learning with Contrastive Predictive Coding* (2018)** — nguồn gốc hàm loss InfoNCE, nền tảng lý thuyết cho cách train hầu hết embedding model hiện đại. Xem phần "Contrastive Learning" trong [08-Embedding.md](../08-Embedding/08-Embedding.md).
- **[Nên đọc] Schroff et al., *FaceNet: A Unified Embedding for Face Recognition and Clustering* (2015)** — nguồn gốc triplet loss, hữu ích để hiểu vì sao các phương pháp mới (InfoNCE) lại thay thế nó ở hầu hết bài toán embedding văn bản. Xem phần "Triplet Loss" trong [08-Embedding.md](../08-Embedding/08-Embedding.md).
...
- **[Bắt buộc] Malkov & Yashunin, *Efficient and Robust Approximate Nearest Neighbor Search Using Hierarchical Navigable Small World Graphs* (2018)** — thuật toán HNSW, cơ chế index mặc định của Qdrant và hầu hết vector DB hiện đại. Nên đọc để hiểu vì sao search trên hàng triệu vector vẫn nhanh — xem phần "Tìm kiếm vector" trong [08-Embedding.md](../08-Embedding/08-Embedding.md).

## Chương 17.4 — RAG

*Song song với [09-RAG.md](../09-RAG/09-RAG.md).*

- **[Bắt buộc] Lewis et al., *Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks* (2020)** — paper đặt tên và định hình kiến trúc RAG. Đáng đọc kỹ vì bản gốc train luôn retriever cùng generator (end-to-end), khác với cách dùng retriever "đóng băng" phổ biến hiện nay ở [09-RAG.md](../09-RAG/09-RAG.md).
...
- **[Nên đọc] Robertson & Zaragoza, *The Probabilistic Relevance Framework: BM25 and Beyond* (2009)** — nền tảng lý thuyết của BM25, vẫn là baseline mạnh cho sparse retrieval. Xem phần "Retriever" trong [09-RAG.md](../09-RAG/09-RAG.md).
...
- **[Bắt buộc] Es et al., *RAGAS: Automated Evaluation of Retrieval Augmented Generation* (2023)** — framework đánh giá RAG tự động (faithfulness, answer relevance, context precision/recall) dùng ở phần "Evaluation" trong [09-RAG.md](../09-RAG/09-RAG.md).

## Chương 17.5 — Agent & Reasoning

*Song song với [11-LangChain.md](../11-LangChain/11-LangChain.md), [12-LangGraph.md](../12-LangGraph/12-LangGraph.md), [13-Agent.md](../13-Agent/13-Agent.md).*

- **[Bắt buộc] Yao et al., *ReAct: Synergizing Reasoning and Acting in Language Models* (2022)** — paper định hình vòng lặp reasoning + action mà gần như mọi agent framework hiện nay (LangGraph, AutoGen...) đều dựa trên. Đọc kỹ trước phần "Agent là gì" trong [13-Agent.md](../13-Agent/13-Agent.md).
- **[Nên đọc] Yao et al., *Tree of Thoughts: Deliberate Problem Solving with Large Language Models* (2023)** — mở rộng ReAct thành tìm kiếm đa nhánh có backtrack, xem phần "Chiến lược Planning nâng cao" trong [13-Agent.md](../13-Agent/13-Agent.md).
...
- **[Tham khảo thêm] Wu et al., *AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation* (2023)** — paper đứng sau framework AutoGen, xem phần "Multi-Agent" trong [13-Agent.md](../13-Agent/13-Agent.md).
- **[Tham khảo thêm] Jimenez et al., *SWE-bench: Can Language Models Resolve Real-World GitHub Issues?* (2023)**, **Mialon et al., *GAIA: A Benchmark for General AI Assistants* (2023)**, **Liu et al., *AgentBench: Evaluating LLMs as Agents* (2023)** — ba benchmark chuẩn để đánh giá agent, xem phần "Agent Evaluation" trong [13-Agent.md](../13-Agent/13-Agent.md).
...
## Chương 17.6 — Alignment & Fine-tuning nâng cao

*Mở rộng thêm cho [10-Fine-Tuning.md](../10-Fine-Tuning/10-Fine-Tuning.md) — sách mới dừng ở LoRA/QLoRA (fine-tune để model học domain/format), còn alignment (dạy model "cư xử đúng ý người") là một chủ đề riêng.*
...
## Chương 17.7 — Hiệu năng & Serving nâng cao

*Mở rộng thêm cho [14-Deployment.md](../14-Deployment/14-Deployment.md) và phần KV Cache trong [06-Transformer.md](../06-Transformer/06-Transformer.md) — các kỹ thuật tối ưu tốc độ/bộ nhớ dùng trong production nhưng sách chưa đi sâu.*
...
## Chương 17.8 — Lịch sử & Scaling Laws
...
- **[Nên đọc] Devlin et al., *BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding* (2018)** — hướng "encoder-only", vẫn là nền tảng của nhiều embedding model hiện nay. Xem so sánh 3 dòng kiến trúc ở [07-BERT-T5-GPT.md](../07-BERT-T5-GPT/07-BERT-T5-GPT.md).
- **[Nên đọc] Radford et al., *Improving Language Understanding by Generative Pre-Training* (2018, GPT-1)** và **Brown et al., *Language Models Are Few-Shot Learners* (2020, GPT-3)** — hai cột mốc cho thấy decoder-only scale lên thì xuất hiện khả năng few-shot mà không cần fine-tune, tiền đề trực tiếp cho prompt engineering ở [06-Transformer.md](../06-Transformer/06-Transformer.md). Xem thêm lịch sử GPT ở [07-BERT-T5-GPT.md](../07-BERT-T5-GPT/07-BERT-T5-GPT.md).
- **[Tham khảo thêm] Raffel et al., *Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer* (2019)** — T5, đại diện hướng encoder-decoder (có cross-attention, xem [06-Transformer.md](../06-Transformer/06-Transformer.md)), hữu ích để so sánh với lựa chọn decoder-only. Xem thêm ở [07-BERT-T5-GPT.md](../07-BERT-T5-GPT/07-BERT-T5-GPT.md).
...
## Chương 17.9 — RAG & Agent nâng cao

*Các biến thể mới hơn RAG/Agent cơ bản đã học ở chương 07, 09-11 — phù hợp khi hệ thống ở [16-Projects.md](../16-Projects/16-Projects.md) cần cải thiện thêm.*

- **[Nên đọc] Asai et al., *Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection* (2023)** — model tự quyết định khi nào cần retrieve (không phải lúc nào cũng retrieve cố định như đã học ở [09-RAG.md](../09-RAG/09-RAG.md)), và tự đánh giá độ liên quan/độ trung thực của câu trả lời bằng các token đặc biệt được train riêng.
...
- **[Nên đọc] Schick et al., *Toolformer: Language Models Can Teach Themselves to Use Tools* (2023)** — model tự học khi nào nên chèn lời gọi tool vào text bằng self-supervised learning, một hướng khác với tool calling native (train sẵn) đã dùng ở phần "Tool Calling" trong [13-Agent.md](../13-Agent/13-Agent.md).
- **[Tham khảo thêm] Park et al., *Generative Agents: Interactive Simulacra of Human Behavior* (2023)** — mô phỏng một "thị trấn ảo" nhiều agent có memory, lập kế hoạch, và tương tác xã hội với nhau — ví dụ mở rộng thú vị cho ý tưởng multi-agent ở [13-Agent.md](../13-Agent/13-Agent.md).

## Chương 17.10 — Xem trước: Diffusion & Multimodal

*Chuẩn bị cho [18-Diffusion.md](../18-Diffusion/18-Diffusion.md) (chưa viết) — 3 paper nền tảng nếu muốn đọc trước.*
...
## Chương 17.11 — Cách đọc một paper AI/ML hiệu quả
...
Với paper toán nặng (như RoPE, InfoNCE), một mẹo hiệu quả: **tự code lại phần công thức cốt lõi** bằng vài dòng NumPy/PyTorch trước khi đọc phần chứng minh toán học đầy đủ — đây chính là cách tiếp cận các ví dụ trong sách này đã dùng (xem phần "RoPE" trong [06-Transformer.md](../06-Transformer/06-Transformer.md), phần "Contrastive Learning" trong [08-Embedding.md](../08-Embedding/08-Embedding.md)) — code chạy được thường làm rõ trực giác nhanh hơn đọc công thức suông.

## Chương 17.12 — Bảng tổng hợp theo dòng thời gian
...
## Chương 17.13 — Tài liệu tham khảo
