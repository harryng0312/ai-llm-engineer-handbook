# 12 — Deployment: Serving, Observability, Platform

> Các chương trước xây model ([04-Transformer.md](../04-Transformer/04-Transformer.md), [08-Fine-Tuning.md](../08-Fine-Tuning/08-Fine-Tuning.md)) và agent ([11-Agent.md](../11-Agent/11-Agent.md)) — nhưng chạy được trên máy dev là một chuyện, đưa vào production là chuyện khác. Chương này bàn về ba mối quan tâm khi đưa hệ thống LLM/agent ra thực tế: **serving** (chạy model hiệu quả, phục vụ nhiều request), **observability** (biết được hệ thống đang làm gì, debug khi có sự cố), và các mối quan tâm ở tầng **platform** khi hệ thống phục vụ nhiều team/tenant thay vì một ứng dụng đơn lẻ.

> 📓 Toàn bộ code ví dụ trong chương này cũng có ở dạng notebook chạy được: [`12-Deployment.ipynb`](./12-Deployment.ipynb).

## Mục lục

1. [Chương 12.1 — Serving: Ollama, vLLM, llama.cpp](#chương-121-serving-ollama-vllm-llamacpp)
2. [Chương 12.2 — Observability](#chương-122-observability)
3. [Chương 12.3 — Agent Platform và Governance](#chương-123-agent-platform-và-governance)
4. [Chương 12.4 — Bài tập](#chương-124-bài-tập)
5. [Chương 12.5 — Tài liệu tham khảo](#chương-125-tài-liệu-tham-khảo)

---

## Chương 12.1 — Serving: Ollama, vLLM, llama.cpp
...
## Chương 12.3 — Agent Platform và Governance
...
**AI Gateway** — một lớp đứng trước mọi request tới LLM/agent, đảm nhiệm auth, rate limiting, logging tập trung — giống vai trò của lớp Rust gateway đã phác thảo ở [13-Rust-AI.md](../13-Rust-AI/13-Rust-AI.md), chỉ khác là ở quy mô nhiều team/nhiều model hơn.
...
- Định kỳ review log để phát hiện dấu hiệu prompt injection đã lọt qua guardrail ở [11-Agent.md](../11-Agent/11-Agent.md).

Việc triển khai đầy đủ Model Routing/AI Gateway/Governance cho một nền tảng nhiều tenant là chủ đề riêng của kiến trúc hệ thống doanh nghiệp — mục này chỉ nhằm giúp nhận diện đúng vấn đề khi dự án cá nhân ở [14-Projects.md](../14-Projects/14-Projects.md) phát triển vượt quy mô một ứng dụng đơn lẻ.

## Chương 12.4 — Bài tập

1. Sửa `generate_with_cache` ở [04-Transformer.md](../04-Transformer/04-Transformer.md) để in ra số token/giây, so sánh với `generate_naive` khi `n_new_tokens` = 20, 50, 100 — vẽ biểu đồ tốc độ theo độ dài chuỗi.
...
3. Bật LangSmith tracing (xem [Chương 12.2](#chương-122-observability)) cho một agent đã xây ở [11-Agent.md](../11-Agent/11-Agent.md), chạy vài câu hỏi cần gọi 2-3 tool, quan sát trace trên dashboard: đếm số bước, so sánh độ trễ giữa các node, xác định node nào chậm nhất.
4. Viết một model router đơn giản (mở rộng `route_model` ở [Chương 12.3](#chương-123-agent-platform-và-governance)) chọn giữa 2 model dựa trên độ dài/độ phức tạp câu hỏi, thử với 10 câu hỏi khác nhau và tự đánh giá router có chọn hợp lý không.
